...
module.exports.run = async (client, message, args) => {
  ...
  async function getData(embed, server) {
    ...
    socket.on("data", async data => {
      ...
      if (dataFromServer.includes("ENDFILES")) {
        await parseData(embed, StrCut(data.toString(), "Players", 0), server);
        socket.end();
      }
    });

    socket.on("error", async () => {
      await parseData(embed, "", server);
      socket.end();
    });
  }
  ...
  async function parseData(embed, serverInformation, server) {
      ...
      if (gamestat.PlayersNum > 0) {
        ...
        if (gamestat.PlayersInfo.spectators.string != "") {
          embed.addField(
            ...
            (await gamestat.PlayersInfo.spectators.string) != ""
    ...
    if (server + 1 < servers.length) await getData(embed, server + 1);
    else update(embed);
  }

  async function update(embed) {
    let fetched = await client.channels
      .get(options.statusChannel_ctf)
      .fetchMessages({ limit: 10 });
    client.channels.get(options.statusChannel_ctf).bulkDelete(fetched);
    client.channels.get(options.statusChannel_ctf).send(embed);
  }
};
